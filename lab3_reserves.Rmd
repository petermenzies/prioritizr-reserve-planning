...
3. Is there a spatial pattern in the planning unit cost values (hint: use `plot` to make a map)?
  ...
  Yes - cost is lowest in the eastern part of the region, highest in the central part, and mid-range in the west.
...
prop_12 <- cellStats(veg_data[[12]], "sum") / ncell(veg_data)

which.max(cellStats(veg_data, "sum"))
```
...
1. What part of the study area is the 13th vegetation class found in (hint: make a map)? For instance, is it in the south-eastern part of the study area?
  ...
  In the east, with the highest concentration in the northeast.
...
sum(repr_data$relative_held < 0.2)

plot(repr_data$relative_held ~ abundance_data$absolute_abundance)

lm(repr_data$relative_held ~ abundance_data$absolute_abundance) %>% summary()
```
...
# verifying all targets were met
which(s1_summary$relative_held < 0.05)
```
...
2. Is there a pattern in the spatial distribution of the priority areas?
  ...
  There appear to be more in the eastern half of the region than in the western half.
...
3. Can you verify that all of the targets were met in the prioritization (hint: `eval_feature_representation_summary(p1, s1[, "solution_1"])`)?
  ...
  Yes. `which(s1_summary$relative_held < 0.05) = 0`
...
```{r}
eval_cost_summary(p1, s1[, "solution_1"])$cost
eval_cost_summary(p2, s2[, "solution_1"])$cost
eval_cost_summary(p3, s3[, "solution_1"])$cost
eval_cost_summary(p4, s4[, "solution_1"])$cost
...
2. How many planning units are in `s2`, `s3`, and `s4`?
  ...
  s2: 205 planning units 
  s3: 211 planning units
  s4: 211 planning units
  
3. Do the solutions with more planning units have a greater cost? Why (or why not)?
  
  Yes. The solutions with more planning units have more stringent parameters (i.e. excluding highly degraded areas and/or higher relative target). The ability of the solution to minimize cost is impacted by those stipulations. 
  
4. Why does the first solution (`s1`) cost less than the second solution with protected areas locked into the solution (`s2`)?
  
  The locked in protected areas entail a large addition to the included planning units and are selected regardless of cost---thus, cost is minimized far less than for `s1` which was created with fewer constraints.
  
5. Why does the third solution (`s3`) cost less than the fourth solution solution with highly degraded areas locked out (`s4`)?
  
  The highly degraded areas that were locked out contained less costly planning units that could no longer be utilized in  `s4`.
...
eval_cost_summary(p6, s6[, "solution_1"])$cost

!identical(s4$solution_1, s6$solution_1)
...
eval_cost_summary(p7, s7[, "solution_1"])$cost

!identical(s4$solution_1, s7$solution_1)

spplot(s7, "solution_1", col.regions = c("grey80", "darkgreen"), main = "s7",
       colorkey = FALSE)

s7_summary <- eval_feature_representation_summary(p7, s7[, "solution_1"])
```
...
1. What is the cost the fourth (`s4`) and fifth (`s5`) solutions? Why does the fifth solution (`s5`) cost more than the fourth (`s4`) solution?
  ...
  The fragmentation penalty in `s5` incurs additional cost by selecting planning units that are not necessarily the least costly in order to reduce boundary. 
...
2. Try setting the penalty value to 0.000000001 (i.e. `1e-9`) instead of 0.001. What is the cost of the solution now? Is it different from the fourth solution (`s4`) (hint: try plotting the solutions to visualize them)? Is this is a useful penalty value? Why (or why not)?
  ...
  $6711.756. It is very slightly different from `s4`. Only a few planning units were selected differently. This does not seem to be a useful penalty value because fragmentation appears to have only been affected marginally, but it still incurred additional, albeit slight, cost.
...
3. Try setting the penalty value to 0.5. What is the cost of the solution now? Is it different from the fourth solution (`s4`) (hint: try plotting the solutions to visualize them)? Is this a useful penalty value? Why (or why not)?
  ...
  $9816.64. This solution is quite different from `s4`. There is no fragmentation---the selected planning units form a contiguous area. This does not appear to be a useful penalty value in this instance because the eastern portion of the region lacks any selected planning units, and the cost is far more than any other solution that has been explored.
